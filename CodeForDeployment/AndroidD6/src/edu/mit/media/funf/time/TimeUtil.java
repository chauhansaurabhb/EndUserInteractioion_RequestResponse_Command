...
public class TimeUtil {
	...
	/**
	 * Returns a BigDecimal timestamp in seconds with millisecond precision,
	 * using System.currentTimeMillis()
	 * 
	 * @return
	 */
	public static BigDecimal getTimestamp() {
		return DecimalTimeUnit.MILLISECONDS.toSeconds(System
				.currentTimeMillis());
	}
	...
	/**
	 * Aligns the nano seconds to the start of a new millisecond. This should be
	 * called whenever device wakes up from sleep.
	 */
	public static void calibrateNanosConversion() {
		...
		TimeUtil.secondsOffset = BigDecimal.valueOf(TimeUtil.referenceMillis,
				MILLI).subtract(
	...
	public static BigDecimal uptimeNanosToTimestamp(long nanos) {
		if (TimeUtil.secondsOffset == null) {
			...
			BigDecimal currentTimeStamp = DecimalTimeUnit.MILLISECONDS
					.toSeconds((currentMillis1 + currentMillis2) / 2.0);
			if (TimeUtil._uptimeNanosToTimestamp(currentNanos)
					.subtract(currentTimeStamp).abs().doubleValue() > TimeUtil.CLOCK_OFFSET_TOLERANCE) {
				calibrateNanosConversion();
			}
		}
	...
	// Round to microseconds, because of the inaccuracy associated with our
	// method of syncing the clocks
	public static BigDecimal _uptimeNanosToTimestamp(long nanos) {
		return roundToMicroPrecision(BigDecimal.valueOf(nanos, NANO).add(
				TimeUtil.secondsOffset));
	}
